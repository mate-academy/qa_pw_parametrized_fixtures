...
articleTagParameters.forEach(({ tagNumber, partOftestName}) =>{
  test(`User delete ${partOftestName} to exist article`, async ({
    page,
    viewArticlePage
  }) => {
    ...
    await viewArticlePage.clickEditArticleButton();
    await editArticlePage.deleteTags(articleTags);
    await editArticlePage.clickUpdateButton();
    await viewArticlePage.refreshPageForUpdateArticlePage();
    await viewArticlePage.assertArticleTagsHidden(articleTags);
  });
